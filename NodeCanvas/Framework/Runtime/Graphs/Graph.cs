...
namespace NodeCanvas.Framework{
	...
	abstract public partial class Graph : ScriptableObject, ITaskSystem, ISerializationCallbackReceiver, IScriptableComponent {
		...
		protected void OnValidate(){ Validate(); } //Validate Graph
		protected void OnEnable(){
			//validate only the first time the graph is enabled. This is done for when the bound graph's owner go is enable/disabled at runtime
			if (!hasEnabled){
				hasEnabled = true;
				Validate();
			}
		...
		public IBlackboard blackboard{
			...
			{
				#if UNITY_EDITOR
				//TODO: Fix this properly (For when the blackboard is a component and has been removed by the user)
			...
			set
			{
				var bb = useLocalBlackboard? localBlackboard : value;
				if (!ReferenceEquals(_blackboard, bb)){
					_blackboard = bb;
					UpdateReferences();
				}
			}
